...
const ExpensesRequest = () => {
  ...
  const getAllRequest = async () => {
    try {
      ...
      let requestList = res.data.requests.filter(
        (item) => item.status === "pending",
      );
      let approved = res.data.requests.filter(
  ...
  return (
    <div className="request">
      ...
      <form onSubmit={handleSubmit}>
        ...
        <div>
          {loading ? (
            <button type="submit" style={{ opacity: 0.4 }} disabled>
              Submit
            </button>
          ) : (
      ...
      <div className="expense-requests">
        {requestList.map((req) => (
          <div key={req._id} className="req-card">
            ...
            <p>
              <strong>Employye Name: </strong>
              {req.requesterName}
            </p>
            <p>
              <strong>Cateory:</strong> {req.category}
            </p>
            <p>
              <strong>Notes: </strong>
              {req.notes}
            ...
            <button
              type="button"
              onClick={() => requestStatus(req._id, "approved")}
            >
              approve
            </button>
            <button
              type="button"
              onClick={() => requestStatus(req._id, "denied")}
            >
              Reject
            </button>
          </div>
